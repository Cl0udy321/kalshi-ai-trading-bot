...
LLM call goes through src/clients/xai_client.py → groq_client.py,
which uses an Groq fallback chain (try the configured model,
...
class DirectionalBot:
    ...
    async def run_trading_mode(self):
        ...
        try:
            ...
            # LLM client — single-model with Groq fallback chain. The
            # XAIClient name is historical; it routes through Groq,
